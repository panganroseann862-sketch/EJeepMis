...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold" style="color:#12196B;">E-Jeep Management</h1>
                <p class="text-gray-500 mt-1">Manage your fleet of e-jeepneys.</p>
            </div>
            <a href="{{ route('admin.ejeeps.create') }}"
                ...
                <table class="min-w-full">
                    <thead>
                        <tr style="background:#F8F9FC; border-bottom: 1.5px solid #E8EAF2;">
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider" style="color:#12196B;">Vehicle Number</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider" style="color:#12196B;">Plate Number</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider" style="color:#12196B;">Capacity</th>
                    ...
                    <tbody>
                        @forelse($ejeeps as $ejeep)
                            <tr style="border-bottom: 1px solid #F0F2F8; transition: background 0.15s, transform 0.15s;"
                                onmouseenter="this.style.background='#F8F9FC';"
                                onmouseleave="this.style.background='{{ $ejeep->trashed() ? '#F8F9FC' : 'white' }}';">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm font-semibold" style="color:#12196B;">
                                            {{ $ejeep->vehicle_number }}
                                        </span>
                                        @if($ejeep->trashed())
                                            <span class="text-xs px-2 py-0.5 rounded font-medium" style="background:#FCEBEB; color:#A32D2D;">(Deleted)</span>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-1 text-sm text-gray-600">
                                        ...
                                        {{ $ejeep->passenger_capacity }} passengers
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @elseif($ejeep->operational_status === 'maintenance')
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold"
                                            style="background:#FAEEDA; color:#633806;">
                                            <span class="w-1.5 h-1.5 rounded-full inline-block" style="background:#EF9F27;"></span>
                                            Maintenance
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if(!$ejeep->trashed())
                                        <div class="flex items-center gap-1">
                                            <a href="{{ route('admin.ejeeps.show', $ejeep) }}"
                                            ...
                                            <a href="{{ route('admin.ejeeps.edit', $ejeep) }}"
                                                class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-medium"
                                            ...
                                            <form action="{{ route('admin.ejeeps.destroy', $ejeep) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-medium"
                                                    ...
                                                    onclick="return confirm('Are you sure you want to delete this E-Jeep?')">
                                                    <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center">
                                    ...
                                    <p class="text-sm text-gray-400">No E-Jeeps found.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
